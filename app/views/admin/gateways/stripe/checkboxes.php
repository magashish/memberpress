...
<table class="form-table">
  <tbody>
      <tr valign="top">
        ...
        <tr valign="top">
          <th colspan="2">
            <div class="mepr-stripe-customize-payment-methods">
              <button type="button" class="button button-secondary mepr-customize-btn" data-modal-target="mepr-stripe-payment-methods-modal-<?php echo esc_attr($id); ?>"><?php esc_html_e('Customize Payment Methods', 'memberpress'); ?></button>
              <div class="mepr_modal" id="mepr-stripe-payment-methods-modal-<?php echo esc_attr($id); ?>" role="dialog" aria-modal="true">
                          ...
                          <?php foreach ($payment_methods as $key => $payment_method) : ?>
                            <div class="mepr-stripe-payment-method">
                              ...
                              <label for="<?php echo esc_attr(sanitize_key("$payment_methods_str-{$payment_method['key']}")); ?>"><?php echo esc_html($payment_method['name']); ?></label>
                                ...
                                <?php if ($key === 0) : ?>
                                    <?php
                                    // These payment methods require domain registration, but aren't separate payment method types
    ...
    <tr valign="top" <?php echo MeprStripeGateway::is_stripe_connect($id) || empty($live_public_key) ? 'style="display:none;"' : ''; ?>>
      <th scope="row"><label for="<?php echo esc_attr($debug_str); ?>"><?php esc_html_e('Send Debug Emails', 'memberpress'); ?></label></th>
      <td><input type="checkbox" name="<?php echo esc_attr($debug_str); ?>"<?php echo checked($debug); ?> /></td>
    </tr>
    <tr valign="top" <?php echo $hide_webhook_url ? 'class="mepr-hidden"' : ''; ?>>
